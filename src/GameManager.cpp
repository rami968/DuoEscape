...
bool GameManager::init(std::vector<std::string>& errors) {
	// Determine asset directory (prefer "assets" folder if present)
	std::string assetDir = ".";
	...
	// Load Riddles
	std::string riddlesPath = assetDir + "/riddles.txt";
	if (!fs::exists(riddlesPath) && assetDir != ".") {
		riddlesPath = "riddles.txt";
	}
	if (!screen::loadRiddlesFromFile(riddlesPath, errors)) {
	...
	if (screenFiles.empty()) {
		errors.push_back("Critical Error: No 'adv-world*.screen' files found in assets or working directory.");
		return false;
	}
...
void GameManager::changeScreen(int newScreenID, const Point& destinationPos, Player& p1, Player& p2) { // Change active screen and update player positions
    ...
    if (newScreenID >= 0 && newScreenID < (int)screens.size()) {
        ...
        if (isFinalScreen) {
            ...
            currentScreen.draw(isColorEnabled);
            _getch();
            return;
        }
...
void GameManager::displayMenu() const {
    ...
    std::cout << "                              (3) Color mode " << (isColorEnabled ? "enabled." : "disabled.") << " Press 3 to change" << std::endl;
...
void GameManager::showMenuAndHandleInput() { // Main menu loop
    ...
    while (running) {
		...
		case static_cast<char>(MenuChoice::ColorMode):
...
void GameManager::checkAndHandleTorchUpdate(Player& p1, Player& p2, bool& lastTorchState) { // Update lighting if torch state changes
    ...
    if (torchActive != lastTorchState) { // Check if lighting state needs updating
        ...
        screens[currentScreenID].draw(isColorEnabled);
...
void GameManager::displayingPlayerStatus(Player& p1, Player& p2, screen& currentScreen) {
    ...
    for (int y = topY; y <= bottomY; ++y) {
        currentScreen.setCharAt(Point(0, y, 0, 0, static_cast<char>(MapChar::Empty)), static_cast<char>(MapChar::Wall));
        currentScreen.setCharAt(Point(screen::MAX_X - 1, y, 0, 0, static_cast<char>(MapChar::Empty)), static_cast<char>(MapChar::Wall));
    }
    ...
    if (clearStartY <= clearEndY) {
        for (int y = clearStartY; y <= clearEndY; ++y) {
            for (int x = 1; x <= screen::MAX_X - 2; ++x) {
                currentScreen.setCharAt(Point(x, y, 0, 0, static_cast<char>(MapChar::Empty)), static_cast<char>(MapChar::Empty));
            }
        }
    }
    ...
    char p1Held = (p1.hasElement() ? p1.getHeldElement() : static_cast<char>(MapChar::NoItem));
    char p2Held = (p2.hasElement() ? p2.getHeldElement() : static_cast<char>(MapChar::NoItem));
    ...
    writeTextAtColumn(line0, COL_PLAYER1_LIVES, "Player $: Lives:" + std::to_string(p1.getLives()));
    writeTextAtColumn(line0, COL_CURRENT_ROOM, "Current Room:" + std::to_string(roomIdToDisplay));
    writeTextAtColumn(line0, COL_PLAYER2_LIVES, "Player &: Lives:" + std::to_string(p2.getLives()));
...
bool GameManager::processBombs(Player& p1, Player& p2) {
    ...
    if (needsRedraw) {
        getCurrentScreen().draw(isColorEnabled);
        p1.draw(); p2.draw();
    }
    return playerHit;
}
...
bool GameManager::explodeBomb(const Bomb& bomb, Player& p1, Player& p2) {
    ...
    for (int dy = -BOMB_RADIUS; dy <= BOMB_RADIUS; ++dy) for (int dx = -BOMB_RADIUS; dx <= BOMB_RADIUS; ++dx) {
        ...
        Point target(x, y, 0, 0, static_cast<char>(MapChar::Empty));
...
void GameManager::tryCollectStar(Player& p) {
    ...
    if (sc.getCharAt(pos) == static_cast<char>(MapChar::Star)) {
        addTeamScore(STAR_POINTS);
        sc.setCharAt(pos, static_cast<char>(MapChar::Empty)); 
    }
}
...
void GameManager::writeLegendLine(screen& sc, int y, const std::string& text) {
    ...
    for (int i = 0; i < LEGEND_WIDTH; ++i) {
        char ch = (i < (int)text.size()) ? text[i] : static_cast<char>(MapChar::Empty);
        sc.setCharAt(Point(baseX + i, y, 0, 0, static_cast<char>(MapChar::Empty)), ch);
    }
}
