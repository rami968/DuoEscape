...
void Player::draw() {
	...
	p.draw(gameManager->getIsColorEnabled());
}
...
void Player::move() {
	...
	p.draw(backgroundChar, gameManager->getIsColorEnabled()); // Redraw background at current position
	Point p_orig = p;
	...
	if (!awaitingScreenTransition) {
		p.draw(gameManager->getIsColorEnabled()); // Draw player at final position
	}
}
...
void Player::disposeElement() {
	...
	if (heldElement == static_cast<char>(MapChar::BombItem)) {
		...
		theScreen->draw(gameManager->getIsColorEnabled());
		p.draw(gameManager->getIsColorEnabled());
		return;
	}
...
// Checks if the player can press a spring
...
// Handles a spring launch
void Player::handleSpringLaunch() {
	char backgroundUnderPlayer = theScreen->getCharAt(p);
	p.draw(backgroundUnderPlayer, gameManager->getIsColorEnabled());

	Direction inputDir = this->p.getDirection(); 

	if (isPerpendicular(inputDir, activeSpringDir)) { // Move player laterally if launch is perpendicular
		Point lateralPos = p.calculateNext(inputDir);

		if (theScreen->isClear(lateralPos, true) && !gameManager->isOtherPlayerAt(lateralPos, this)) { // Move player laterally if the next position is clear
			p = lateralPos;
		}
	}
	...
	for (int i = 0; i < this->springSpeed; ++i) { 
		...
		if (gameManager->isOtherPlayerAt(nextPos, this)) { // Transfer launch if another player is in the way
		...
		Point originalPos = p;
		if (theScreen->isClear(nextPos, true)) { // Move player if the next position is clear
			p = nextPos;
			handleInteractions(originalPos); // Handle interactions at the new position
			if (p == originalPos) {
				break;
			}
			if (!isBeingLaunched) {
		...
		else { // Block launch if the next position is not clear
			this->springTimer = 0; 
			this->isBeingLaunched = false;
			this->springSpeed = 1;
			this->p.setDirection(Direction::STAY);
	...
	if (this->springToReset != nullptr) {  // Reset spring if it was used
	...
	this->springTimer--; 
	if (this->springTimer <= 0) {  // End launch if timer runs out
		this->isBeingLaunched = false;
		this->springSpeed = 1;
		if (isPerpendicular(inputDir, activeSpringDir)) { // Set direction back to input direction if launch is perpendicular
			this->p.setDirection(inputDir);
			this->lastMoveDir = inputDir;
		}
		else { // Set direction to stay if launch is not perpendicular
			this->p.setDirection(Direction::STAY);
			this->lastMoveDir = Direction::STAY;
		}
	}
...
// Returns the direction corresponding to a key
...
// Receives a launch from a spring
	...
	if (theScreen->isWall(p)) {
		...
		if (currentObstacle) {
			...
			if (gameManager->canObstacleMove(currentObstacle, pushDir, this)) {
				...
				if (secondObs) secondObs->moveObstacle(pushDir, gameManager->getIsColorEnabled());
				currentObstacle->moveObstacle(pushDir, gameManager->getIsColorEnabled());
